...
export async function fetchMenuSectionReport({ search = "", cuisineTypeId = null, locationTypeId = null, isTop200 = null } = {}) {
  const params = new URLSearchParams();
  if (search) params.set("search", search);
  if (cuisineTypeId) params.set("cuisineTypeId", String(cuisineTypeId));
  if (locationTypeId) params.set("locationTypeId", String(locationTypeId));
  if (isTop200 === true || isTop200 === false) params.set("isTop200", String(isTop200));
  const res = await fetch(`${API_BASE}/api/menu-section-report?${params}`);
  if (!res.ok) throw new Error(`Failed to fetch menu section report: ${res.statusText}`);
  return res.json();
}
