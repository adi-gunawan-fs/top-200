...
export async function fetchAnalysisJobs(beforeRecordId, afterRecordId) {
  const { data, error } = await supabase
    .from(TABLE)
    .select("id, before_record_id, after_record_id, item_id, item_type, status, error_message, started_at, completed_at, updated_at")
...
export async function enqueueAnalysisJobs({ beforeRecordId, afterRecordId, jobs }) {
