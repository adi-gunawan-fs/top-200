...
export async function fetchAnalysisBulkRuns(beforeRecordId, afterRecordId) {
  const { data, error } = await supabase
    .from(TABLE)
    .select("id, before_record_id, after_record_id, status, total_items, queued_count, processing_count, completed_count, failed_count, started_at, completed_at, created_at, updated_at")
